  ...
  for i:=0 to FField.FieldCount -1 do begin
    if FField.Visible[i] then begin
         ...
         with TTextView(Flayout.getChildAt(Flayout.getChildCount - 1)) do begin
          ...
          {  case FField.DataType[i] of
              ftInteger, ftFloat: getWindow.setSoftInputMode(ATInputType.TYPE_CLASS_PHONE );
              else
                getWindow.setSoftInputMode(ATInputType.TYPE_TEXT_FLAG_CAP_CHARACTERS);
            end; }
        ...
        if FField.ReadOnly[i] then
           ...
           TDBEditText(Flayout.getChildAt(Flayout.getChildCount - 1)).selectAll;
        end;
    end;
  end;
...
begin
  lay:= AWLinearLayout.Create(aContext);
  lay.setOrientation(AWLinearLayout.VERTICAL);
  ...
  lay.addView(TTextView.create(aContext));
  TTextView(lay.getChildAt(lay.getChildCount - 1)).Text :=  ATHtml.fromHtml(tHTMLTemplate); //tHTMLTemplate;
  Result := lay;
end;
